...
function E04_8() {
    // companies     = axios.get ("http://localhost:3001/companies")
    const companies = [
    ...
    const newObject = (({ name, category }) =>
        ({ name, category, print() { console.log(this.name); } }))
        (companies[0]);
    ...
    return (
        <div>
            ...
            retailCompanies.map(company => (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px', border: '1px solid black', padding: '10px' }}>
